...
class SceauSocket
{
	...
	public function buildHeader()
	{
		...
		} elseif ($this->method == 'GET')
		{
			if (Tools::strlen($this->path.$this->data) > 2048)
				insertLogSceau(get_class($this).' : __construct', 'Maximum length in get method reached('.Tools::strlen($this->path.$this->data).')');
	...
	public function connect($header)
	{
		...
		if ($socket !== false)
		{
			...
			{
				insertLogSceau(__METHOD__.' : '.__LINE__, 'Envoi des données impossible sur : '.$this->host);
				$res = false;
			}
		...
		{ //if connection failed, log
			insertLogSceau(__METHOD__.' : '.__LINE__, 'Connexion socket impossible sur lhôte '.$this->host.'Erreur '.$this->errno.' : '.$this->errstr);
			$res = false;
		}
